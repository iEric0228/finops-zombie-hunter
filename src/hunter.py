"""FinOps Zombie Hunter Lambda: Scans AWS resources for unused (zombie) EBS, RDS, NAT Gateways, and Elastic IPs to estimate monthly savings."""
import os
...
def check_rds_zombies(rds_client, cw_client, dry_run):
    ...
    for db in instances["DBInstances"]:
        ...
        if not metrics["Datapoints"] or all(d["Average"] == 0 for d in metrics["Datapoints"]):
            ...
            print(f"[RDS Zombie] : {db_id} has had zero connections in the past week. Potential saving: ${instance_saving}/month.")
    return savings
...
def check_nat_gw_zombies(ec2_client, cw_client, dry_run):
    ...
    nat_gateways = ec2_client.describe_nat_gateways(Filters=[{"Name": "state", "Values": ["available"]}])["NatGateways"]
    savings = 0
    for nat in nat_gateways:
        ...
        if not metrics["Datapoints"] or metrics["Datapoints"][0]["Sum"] == 0:
            savings += 32
            print(f"[NAT Gateway Zombie] : {nat_id} has had zero bytes out in the past week. Potential saving: $32/month.")
    return savings
